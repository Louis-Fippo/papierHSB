...
\modCG{[comment] In Fig. 1 can we put ``RSTC Network'', ``PH Model'', ``PINT simulator'', and ``r and sa'' in parenthesis ?}
...
%[CG: it is too much detail and redundant]
%\begin{itemize}
...
\modCG{[comment] I think definition 1 is not necessary}
...
\caption{{\bf  Interaction graph linking E-cadherin with 12 genes of the time-series dataset.} Nodes in blue correspond to E-cadherin entities, in red or green, to time-series genes, 
and nodes in It is composed of $293$ nodes and $375$ edges (interactions).
...
\begin{figure}[!ht]
\centering
\includegraphics[width=3.5in]{images/12genes.png}
\caption{\bf 12 gene expressions upon Calcium stimulation.} The $X$ axis represents time duration of the experiment measured in hours. 
...
\modCG{In order to model the fact that a molecule in the interaction graph is influenced by various molecules, two 
...
4 processes. Then, component $c$ is activated ($c_1$ bounces to $c_2$ or $c_0$ bounces to $c_1$) if either  $a$ \modCG{or} $b$ are activated. Therefore, each one of 
...
\modCG{[comment] I think the Example below can be removed.}
...
\modCG{[comment] this part needs to be rewritten, algorithms pseudo-code should be removed.}
...
%To simulate an expected behavior we consider the probability of a reaction to occur and 
%stochastic parameters.
% in the aim at observing an expected behavior. 
...
% or the temporal parameter because $t=r^{-1}$ if we assume that $t$ is the average time of playing an action,
and (2) the stochasticity absorption factor $sa$, which is introduced to control the \modCG{\underline{higher variance 
around the mean duration of playing an action}} \modCG{[comment] I suggest to replace the underlinde last sentence by: ``the variance of firing an action''}. 
...
\begin{tikzpicture}[scale = 0.65]
	      ...
	      %\pause
	      \draw[thick,|<->|] (3,0)node[above,blue]{$MinLevel$} -- (3,3.5);
	      %\pause
...
\caption{\bf Estimation of temporal parameters from time series data:} $r_{i}=\frac{1}{t_{i}-t_{i-1}}$. $MaxLevel$ represents the maximum expression. $MinLevel$ represents the minimum 
...
%These two parameters are estimated from the expression profiles of the time-series data (see Section \ref{SECTSD}). 
