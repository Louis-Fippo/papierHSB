...
% introduced to control the \modCG{\underline{higher variance 
...
\modCG{[comment] It will be good if in Fig 5 we have 0, 1, 2 instead of 1,2, 3}
...
\begin{tikzpicture}[scale = 0.7]
	      ...
	      \foreach \y in {1,2,3}
	      \draw[very thick] (2pt,\y)--(-2pt,\y) node[left,black]{\small\y};
...
component (gene) change of sub-state is estimated as $r_{i}=\frac{1}{t_{i}-t_{i-1}}$. $MaxLevel$ represents the maximum expression of a gene, while 
 $MinLevel$, its minimum expression.
The thresholds $th1$ and $th2$ define the PH discrete sub-states (e.g. 1,2,3) of a component according to its gene expression data.}
...
%The goal of this method is to determine, according to the gene expression level, when a PH component is activated or inhibited.
...
%$\{0, 1, 2\}$. This choice was made because 
...
level $2$. 
%Thus we can automatically determine the different levels of expression of the TSD. 
...
\modCG{[comment] Column ``parameters'' in Figure 6 should be shifted down}
