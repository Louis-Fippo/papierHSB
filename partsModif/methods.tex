...
\end{itemize}
\end{definition}
In this definition terminal nodes can be either \modCG{mRNA expression}, proteins,  complexes, cellular states, biological processes or positive conditions. 
...
nodes (transcriptions, translocations, modifications and compounds). The set of edges is composed of interactions of type activation, inhibition, output, 
...
This pattern models the activation of the component b by the component a.\tabularnewline
\hline
...
This pattern models the inhibition of the component b by the component a\tabularnewline
\hline
...
This pattern models either the activation of the component c by the component a or the inhibition of the 
...
component ( \modCG{mRNA expression}) change of sub-state is estimated as $r_{i}=\frac{1}{t_{i}-t_{i-1}}$. $MaxLevel$ represents the maximum level of a \modCG{mRNA expression}, while 
...
\begin{itemize}
 ...
 choice was made in order to take into account the average time of the Calcium stimuli effect.
...
the percentage of accepted traces is $\frac{Trace_{accp}}{Trace_{N}}$ if  $Trace_{N}$ is the total 
