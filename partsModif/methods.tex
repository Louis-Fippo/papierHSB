...
\modCG{ The studies \cite{AMSW2012,RAT2006} have proposed more elaborated methods for parameter estimation from gene expression data. These methods are well adapted in the case of biochemical reactions where 
...
activity in early hours [$0h$-$5h$] and a low level in late hours [$5h$-$10h$]. \modCG{ This trend has been confirmed by using the SMA (Simple moving average) function of 
the R package TTR which allows us
to smooth time-series data. We used the SMA function whith paramter $n=2$, as a result we obtained that more than $50\%$ of the measured time-series expressions presented these two levels of activities.}
...
%\end{comment}
 ...
 \modCG{
 Due to the stochastic and concurrent aspects of the system, each execution of the model can generate dynamic expression traces which differ.
 Therefore, to validate the proposed model, we analyzed the traces generated by each component for a set of 100 simulations of the model.
We wanted to compute the percentage of traces that reproduce the expected dynamic of the system. To achieve this goal, we take each trace generated
 at each execution for a given component and passed it to an automata ($\mathcal{A}_{i}$) that recognize the set of acceptance for the traces of that component. 
The automata encodes the expected traces from the experimental data.  Thus we can count the 
 number of accepted traces ($Trace_{accp}$). Therefore the percentage of accepted traces is $\frac{Trace_{accp}}{Trace_{N}}$ if we assume that $Trace_{N}$ is the total 
